...
if __name__ == "__main__":
    ...
    print("Starting with Digits Dataset tests")
    """ errorlist = []
    subsetsT = []
    subsetsX = []
    subsetsL = []
	...
	traindigits = pickle.load(fp)
    with open("../Dataset/digitTestset.pickle", "rb") as fp:
	testdigits = pickle.load(fp)
    # generate the 50 subsets used for process
    for i in range(50):
        subClass1 = traindigits[i*20:(i+1)*20]
        subClass2 = traindigits[1001+i*20:1001+(i+1)*20]
        subs = subClass1+subClass2
        X = []
        L = []
        for i in subs:
            X.append(i[0])
    ...
    for i in testdigits:
        ...
        L_test.append(i[1])
    for i in range(1,51):
        X_train_l = subsetsX[i-1:i][0]
        L_train_l = subsetsL[i-1:i][0]
        X_train_u = []
        unlab1 = subsetsX[:i-1]
        unlab2 = subsetsX[i:]
        unlabt = unlab1+unlab2
        for sl in unlabt:
            for item in sl:
                X_train_u.append(item)
        t_start = time.time()
    ...
    for e in errorlist:
        ...
        sq.append(t)
    nmean = np.mean(sq)
    nmean = math.sqrt(nmean)
    print(nmean)
    print()"""
 

    print("Starting with TextDataSet Experiments")
    ...
    test_x = []
    for t in test:
        test_x.append(t[0].toarray().reshape(1,-1)[0])
        test_l.append(t[1])
    X_test = test_x
    L_test = test_l
    for n in numbers:
        ...
        for kkk in range(100):
            ...
            for x in trainSample:
                X_train_l.append(x[0].toarray().reshape(1,-1)[0])
                L_train_l.append(x[1])
            X_train_u = []
            traint = train1 + train2
            for x in traint:
                X_train_u.append(x[0].toarray().reshape(1,-1)[0])
            t_start = time.time()
            model = QN_S3VM(X_train_l, L_train_l, X_train_u, my_random_generator, lam=0.00390625, lamU=1)
            model.train()
            t_end = time.time()
        ...
        mean = np.mean(trainerrors)
        sq = []
        for e in trainerrors:
            t = (e - mean) ** 2
            sq.append(t)
        nmean = np.mean(sq)
        nmean = math.sqrt(nmean)
        errorlist.append([n,mean,nmean])
    print("Results Textclassification")
    for e in errorlist:
        print(e)
    print()
